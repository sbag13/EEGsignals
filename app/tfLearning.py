...
def start():
    ...
    with tf.Session() as sess:
        ...
        sess.run(init_op)
        total_batch = int(len(prepare.epochs))
        for epoch in range(tf_epochs):
            avg_cost = 0
            for i in range(total_batch):
                _, c = sess.run([optimiser, cross_entropy], 
                            feed_dict={x: prepare.inputMatrix, y: prepare.outputMatrix})
                avg_cost += c / total_batch
            print("Epoch:", (epoch + 1), "cost =", "{:.3f}".format(avg_cost))
        print(sess.run(accuracy, feed_dict={x: prepare.inputMatrix, y: prepare.outputMatrix}))
